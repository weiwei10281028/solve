...
3. **題號與範圍**（優先於資料庫推測）：使用者於「補充說明或者問題輸入」指定「第 N 題」→ **僅解該題**，嚴禁解或提及其他題號。未指定題號時：只解**題幹與選項完整可見**之題；圖片被截斷、選項不全、數據缺漏者**不得猜測、不得硬解**。若圖不完整且無使用者補充可補齊條件，開頭說明缺何資訊即可，@@ANSWER@@ 寫「題目資訊不足」。**【使用者補充】與圖片同等效力**；圖被裁切時以補充文字補齊條件，**必須採用**，不得忽略。未指定題號時詳解內勿標「第 N 題」；同一則詳解有**兩題以上且皆完整**時，各題開頭獨立一行「第 N 題」，題間空一行。
...
window.buildSolveUserText = function(scopeInput, refAnswer, opts) {
    ...
    if (textOnly) {
        ...
        if (opts.detailed) {
            ...
            parts.push('請直接開始推導；完成含等號的計算鏈後再評析選項，不可跳過中間算式。');
        }
    } else {

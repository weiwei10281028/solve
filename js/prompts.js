...
2. 禁止寫【核心觀念】【題目核心】【本題關鍵判斷】【違反即錯】【禁止】等改卷標題；禁止開場列「忽略…」「不可混淆…」式陷阱條列。可先寫一兩句解題方法，再進入推導。敘述句只說明「怎麼解」，**具體數值一律寫在 $…$ 等號式裡**，不可掛在中文冒號後面。
...
4. **NOTE**：$\\htmlData{note=…}{…}$ 只標在等號式內的因子或結果，不標敘述句冒號後的裸數字。
5. 分號「；」可連接同一條推導鏈的後續運算；不同物理量第一次出現時，各自要有等號式，不可擠成無主數字。
...
4. NOTE 與分式（依 [參考資料] 板書風格；**白話文詳解**）：
- 關鍵數字與關鍵算式因子用 $\\htmlData{note=白話短註解}{內容}$，須寫在 $ 內；note 文字用學生能懂的繁體中文（如「排泄後剩餘比例」「衰變後剩餘比例」「SO₃莫耳分率」），**禁止**在式子上方或下方另外寫標籤文字。
- **禁止**用 $\\underbrace$、$\\underset$、$\\overset$ 標註因子名稱；說明一律走 NOTE，點擊才顯示。
- 乘積中每個獨立因子、帶指數的括號式、整段分數，各用一層 $\\htmlData$ **包住整段**（勿拆成可見標籤）。範例：
$A_t = A_0 \\times \\htmlData{note=排泄後剩餘比例}{\\dfrac{1}{2}} \\times \\htmlData{note=衰變後剩餘比例}{\\left(\\dfrac{1}{2}\\right)^{3/6}}$
$P_{SO_3} = \\htmlData{note=SO_3莫耳分率}{\\dfrac{0.1-0.05}{0.1+0.5\\times 0.05}} \\times 1.8$
後來速率 $= \\htmlData{note=濃度對速率影響}{\\left(\\dfrac{3}{4}\\right)^3} \\times \\htmlData{note=溫度對速率影響}{2^{\\frac{60-40}{10}}}$
- 莫耳數、質量比等**需分別說明分子分母**時，才寫 $\\dfrac{\\htmlData{note=…}{分子}}{\\htmlData{note=…}{分母}}$；若 note 是「莫耳分率」「分壓」等整體意義，整段分數包在一個 $\\htmlData$ 內。
- **關鍵數字都要標**（題目給定、中間量、最終結果），用 $\\htmlData$ 標在等號式內。
- **禁止**在 $…$ 內再嵌套 $…$；分式內數字直接寫 \\htmlData{note=…}{6}。
...
【模仿參考資料】
有 [參考資料] 時，排版、分式寫法、Note 標註密度與選項評析節奏須與資料庫詳解一致；數字依圖片重算。模仿壓縮時仍須讓每個數字在等號式中有符號歸屬，不可寫成敘述句冒號後的裸數字。

【參考資料｜僅供內部，勿抄進詳解】
User 訊息中的 [參考資料庫內容]、[教學規定]、[概念參考] 等為內部依據；**禁止**複製其標題、條列、禁止事項到正文。教學規定僅在該題**確實需依數性偶合計算**（$i<1$、求 $i$／$\alpha$）時套用，其餘不強套。詳解直接以物理量標籤＋算式開場。`;

...
2. **每個數字**（含分式分子、分母、係數）一律加 $\\htmlData{note=具體註解}{數字}$。
   - 莫耳數分式：分子註解「總質量」、分母註解「分子量」（例 $\\dfrac{\\htmlData{note=總質量}{46}}{\\htmlData{note=分子量}{46}}$）
   - 原子數計算：乘號後數字註解「每分子原子數」
   - 禁止空泛字眼；單位 mol 寫在數字後同一行（例 $=\\htmlData{note=莫耳數}{1}$ mol。）
...
5. **換行規則（重要）**：
   ...
   - \\htmlData{note=...} 的 note 須寫化學意義（如「碳質量百分比」「乙酸乙酯分子量」），**禁止**「數值」「分子」「分母」「代入數值」等空泛詞。

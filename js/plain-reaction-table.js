...
function mergeAdjacentInlineMath(raw) {
  ...
  for (let pass = 0; pass < 6; pass++) {
    ...
    s = s.replace(/\$([^$\n]+?)\$\s*\$([^$\n]+?)\$/g, (m, a, b) => {
      ...
      if (/=/.test(aTrim) && /=/.test(bTrim)) return `$${aTrim}$，$${bTrim}$`;
      if (/^\\(?:dfrac|frac|tfrac)/.test(bTrim) || /^[\d.]*[A-Za-z]/.test(bTrim) || /^\d/.test(bTrim)) {
        return `$${aTrim}$，$${bTrim}$`;
      }
      return m;
    });
...
function expandDataRowToReactionGrid(reactionCells, dataCells) {
  ...
  for (const v of rawVals) {
    ...
    if (/^[^,\\]+,\s*[^,\\]+$/.test(tv) && !/\\text|\\frac/.test(tv)) {
      vals.push(...tv.split(',').map((s) => s.trim()).filter(Boolean));
    } else {
      vals.push(tv);
    }
  }
  while (vals.length < speciesIdx.length) vals.push('');
  vals.length = speciesIdx.length;
...
/** 反應表列標類型（容許 \text{起始 (M)} 等後綴；勿用 \b，CJK 後接空格會失效） */
function reactionRowLabelKind(row) {
  const s = String(row || '');
  if (/\\text\{[^}]*(?:起始|初始|初)/.test(s) || /^(?:起始|初始|初)\s*&/.test(s)) return 'start';
  if (/\\text\{[^}]*(?:完全向右|完全反應右|移至右)/.test(s) || /\\text\{[^}]*右/.test(s)) return 'complete-right';
  if (/\\text\{[^}]*(?:再向左|完全向左|移至左)/.test(s) || /\\text\{[^}]*左/.test(s)) return 'back-left';
  if (/\\text\{[^}]*(?:變化|變)/.test(s)) return 'change';
  if (/\\text\{[^}]*(?:結果|平衡|平)/.test(s)) return 'result';
  return 'unknown';
}

function isResultLikeRow(row) {
  return reactionRowLabelKind(row) === 'result';
}

function isStartLikeRow(row) {
  return reactionRowLabelKind(row) === 'start';
}

function buildBalanceRowFromSnapshotAndShift(reactionRow, snapshotRow, shiftRow) {
  const rCells = splitArrayRowCells(reactionRow);
  const snapCells = splitArrayRowCells(snapshotRow);
  const shiftCells = splitArrayRowCells(shiftRow);

  if (isGridAlignedDataRow(rCells, snapCells) && isGridAlignedDataRow(rCells, shiftCells)) {
    const out = snapCells.map((s, i) => {
      if (i === 0 && /\\text\{/.test(s)) return '\\text{平衡}';
      if (!s && !shiftCells[i]) return '';
      if (isOperatorCell(rCells[i])) return '';
      return mergeStartChangeCells(s, shiftCells[i]);
    });
    if (/\\text\{/.test(out[0] || '')) out[0] = '\\text{平衡}';
    return out.join(' & ');
  }

  const speciesIdx = getReactionSpeciesColumnIndices(rCells);
  const snapHas = /\\text\{/.test(snapCells[0] || '');
  const shiftHas = /\\text\{/.test(shiftCells[0] || '');
  const snapVals = (snapHas ? snapCells.slice(1) : snapCells).filter((v) => String(v).trim() !== '');
  const shiftVals = (shiftHas ? shiftCells.slice(1) : shiftCells).filter((v) => String(v).trim() !== '');
  const balVals = [];
  for (let j = 0; j < speciesIdx.length; j++) {
    balVals.push(mergeStartChangeCells(snapVals[j] || '', shiftVals[j] || ''));
  }
  return ['\\text{平衡}', ...balVals].join(' & ');
}

/** 反應表結構正規化：保留多列；缺結果／平衡列時自動補列 */
function ensureReactionTableStructure(rows) {
  ...
  const hasStart = bodyRows.some(isStartLikeRow);
  const hasResult = bodyRows.some(isResultLikeRow);
  const hasCompleteRight = bodyRows.some((r) => reactionRowLabelKind(r) === 'complete-right');
  const changeRows = bodyRows.filter((r) => reactionRowLabelKind(r) === 'change' || reactionRowLabelKind(r) === 'back-left');
  const backLeftRow = bodyRows.find((r) => reactionRowLabelKind(r) === 'back-left');
  const changeOnlyRows = bodyRows.filter((r) => reactionRowLabelKind(r) === 'change');

  if (!hasResult && hasStart && hasCompleteRight && (backLeftRow || changeOnlyRows.length === 1)) {
    const completeRow = bodyRows.find((r) => reactionRowLabelKind(r) === 'complete-right');
    const shiftRow = backLeftRow || changeOnlyRows[0];
    if (completeRow && shiftRow) {
      bodyRows = [...bodyRows, buildBalanceRowFromSnapshotAndShift(reaction, completeRow, shiftRow)];
    }
  } else if (!hasResult && hasStart && changeRows.length === 1 && bodyRows.length <= 3) {
    const startRow = bodyRows.find(isStartLikeRow);
  ...
  } else if (!hasResult && hasStart && bodyRows.length === 2) {
    const startRow = bodyRows.find(isStartLikeRow) || bodyRows[0];
    const changeRow = bodyRows.find((r) => r !== startRow) || bodyRows[1];
    if (startRow && changeRow) {
      bodyRows = [...bodyRows, buildResultRowFromStartChange(reaction, startRow, changeRow)];
    }
  }
...
function trimCellSlice(cells, from, to) {
  let s = Math.max(0, from);
  let e = Math.min(cells.length, to);
  while (s < e && !String(cells[s] || '').trim()) s++;
  while (e > s && !String(cells[e - 1] || '').trim()) e--;
  return cells.slice(s, e);
}

function reactionArrowColumnIndices(cells) {
  const idx = [];
  for (let i = 0; i < cells.length; i++) {
    if (/\\rightleftharpoons|\\rightarrow/.test(String(cells[i] || ''))) idx.push(i);
  }
  return idx;
}

/** 第一列含兩個以上 ⇌/→ 時，拆成兩個獨立 array 區塊 */
function splitDualReactionArrayBlock(full) {
  ...
  const dataRows = rows.filter((r) => !isHlineRow(r));
  if (dataRows.length < 2) return full;

  const r0cells = splitArrayRowCells(dataRows[0]);
  const arrowIdx = reactionArrowColumnIndices(r0cells);
  if (arrowIdx.length < 2) return full;

  const a0 = arrowIdx[0];
  const a1 = arrowIdx[1];
  let end1 = a0 + 1;
  while (end1 < a1 && String(r0cells[end1] || '').trim()) end1++;
  let start2 = end1;
  while (start2 < a1 && !String(r0cells[start2] || '').trim()) start2++;
  if (start2 >= a1) return full;

  const splitAt = start2;
  const rows1 = [];
  const rows2 = [];
  ...
  for (const row of rows) {
    ...
    const cells = splitArrayRowCells(row);
    const isReaction = /\\rightleftharpoons|\\rightarrow/.test(row);
    let b1 = trimCellSlice(cells, 0, splitAt);
    let b2 = trimCellSlice(cells, splitAt, cells.length);
    if (!isReaction && b1[0] && /\\text\{/.test(b1[0]) && b2.length && !/\\text\{/.test(b2[0])) {
      b2 = [b1[0], ...b2];
    }
    if (b1.length) rows1.push(b1.join(' & '));
    if (b2.length) rows2.push(b2.join(' & '));
  }

  const data1 = rows1.filter((r) => !isHlineRow(r));
  const data2 = rows2.filter((r) => !isHlineRow(r));
  if (data1.length < 3 || data2.length < 3) return full;

  function alignSpecForRows(rowList) {
    let max = 1;
    for (const r of rowList) {
      if (isHlineRow(r)) continue;
      max = Math.max(max, splitArrayRowCells(r).length);
    }
    return 'c'.repeat(max);
  }

  const spec1 = alignSpecForRows(rows1);
  const spec2 = alignSpecForRows(rows2);
  return [
    `$$\\begin{array}{${spec1}}\n${rows1.join(' \\\\\n ')}\n\\end{array}$$`,
    `$$\\begin{array}{${spec2}}\n${rows2.join(' \\\\\n ')}\n\\end{array}$$`
  ].join('\n\n');
...
function splitPlainReactionArray(tex) {
  ...
  if (dataRows.length - 1 < 3) return null;
...
function injectReactionTableHtml(text) {
  ...
  let s = String(text || '').replace(/\$\$([\s\S]*?)\$\$/g, (full) => {
    if (!/\\begin\{array\}/.test(full) || !/\\rightleftharpoons|\\rightarrow/.test(full)) return full;
    return splitDualReactionArrayBlock(full);
  });
  return s.replace(/\$\$([\s\S]*?)\$\$/g, (full) => {
    if (!/\\begin\{array\}/.test(full) || !/\\rightleftharpoons|\\rightarrow/.test(full)) return full;

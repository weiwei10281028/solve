/**
 * 化學計量引擎網站端 Adapter：第一次 AI 解析 JSON → solveChemistry() → 標準結果。
 */
(function (global) {
  'use strict';

  const ENGINE_ID = 'stoichiometry';
  ...
  function toEngineRequest(parseJson) {
    const parsed = asObject(parseJson, 'parse');
    if (!parsed.ok) return parsed;

    const data = parsed.value;
    if (data.applicable === false) {
      return fail('NOT_APPLICABLE', String(data.unsupportedReason || data.reason || '此題不適用化學計量計算引擎'));
    }
    ...
    const inputObj = normalizeInput(data);
  ...
  function run(parseJson) {
    const mapped = toEngineRequest(parseJson);
  ...
  global.StoichiometryEngineAdapter = api;

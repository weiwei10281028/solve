...
export function solveStoichiometry(request) {
    try {
        ...
        switch (request.operation) {
            ...
            case "percent_yield":
                ...
                break;
            default:
                throw new EngineError("UNSUPPORTED_OPERATION", `不支援 operation: ${request.operation}`, "operation");
        }
        return {
            success: true,
        ...
        const e = error instanceof EngineError
            ? error
            : new EngineError("CALCULATION_ERROR", error instanceof Error ? error.message : "未知錯誤");
        return {
            success: false,
            engine: "stoichiometry",
            operation: request.operation,

...
function readSpecies(input) {
    const species = typeof input.species === "object" && input.species !== null
        ? input.species
        : input;
    const H2A = positiveNumber(species.H2A, "input.species.H2A");
    const HA_minus = positiveNumber(species.HA_minus ?? species["HA-"], "input.species.HA_minus");
    const A_2minus = positiveNumber(species.A_2minus ?? species["A2-"], "input.species.A_2minus");
    let H;
 ...
 */
export function deriveDiproticAcidConstants(input) {
    ...
    const ka1Residual = Math.abs((state.H * state.HA_minus) / state.H2A - Ka1);
    const ka2Residual = Math.abs((state.H * state.A_2minus) / state.HA_minus - Ka2);
    return {
        ...
        checks: {
            allConcentrationsPositive: state.H2A > 0 && state.HA_minus > 0 && state.A_2minus > 0 && state.H > 0,
            Ka1Positive: Ka1 > 0,
            Ka2Positive: Ka2 > 0,

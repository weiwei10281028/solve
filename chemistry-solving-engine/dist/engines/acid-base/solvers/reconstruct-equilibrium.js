 ...
 */
export function reconstructDiproticEquilibrium(input) {
    const formalConcentration = positiveNumber(input.formalConcentration ?? input.concentration, "input.formalConcentration");
    const degreeOfDissociation = fraction(input.degreeOfDissociation ?? input.firstDissociationAlpha ?? input.alpha, "input.degreeOfDissociation");
    const ratioHAtoA2 = positiveNumber(input.ratioHAtoA2 ?? input.haToA2Ratio ?? input.speciesRatio, "input.ratioHAtoA2");
    ...
    const H = 10 ** -pH;
    if (H2A <= 0) {
        throw new EngineError("INVALID_STATE", "重建後 [H2A] 必須大於 0，否則無法反推 Ka1。", "input.degreeOfDissociation");
    }
    ...
    return {
        ...
        checks: {
            concentrationNonnegative: H2A >= 0 && HA_minus >= 0 && A_2minus >= 0 && H >= 0,
            ...
            degreeOfDissociationPassed: Math.abs(alphaCalculated - degreeOfDissociation) < 1e-12,
            degreeOfDissociationResidual: alphaCalculated - degreeOfDissociation,
            chargeBalancePassedForPureAcid: Math.abs(chargeBalanceDifference) < 1e-8,

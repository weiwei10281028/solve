...
import {
  ...
  signedStoichiometricCoefficient,
} from "../validation.js";
import { calculateReactionQuotient, compareQToK, type EquilibriumDirection } from "./reaction-quotient.js";

function opposite(direction: EquilibriumDirection): EquilibriumDirection {
  if (direction === "forward") return "reverse";
  if (direction === "reverse") return "forward";
  return "at_equilibrium";
}

export function solveLeChatelierDisturbance(input: Record<string, unknown>) {
  const reaction = reactionDefinition(input.reaction);
  const disturbance = oneOf(
    input.disturbance,
    ["concentration_change", "volume_change", "pressure_change", "temperature_change", "add_inert_gas"] as const,
    "input.disturbance",
  );

  let equilibriumShift: EquilibriumDirection = "at_equilibrium";
  let method = "le_chatelier_stoichiometric_rule";
  const intermediates: Record<string, unknown> = { disturbance };
  const warnings: string[] = [];

  if (disturbance === "concentration_change") {
    const species = nonemptyString(input.species, "input.species");
    const change = oneOf(input.change, ["increase", "decrease"] as const, "input.change");
  ...
  if (disturbance === "volume_change" || disturbance === "pressure_change") {
    ...
    if (deltaGasMoles === 0) {
      ...
      const moreGasDirection: EquilibriumDirection = deltaGasMoles > 0 ? "forward" : "reverse";
  ...
  if (disturbance === "temperature_change") {
    const change = oneOf(input.change, ["increase", "decrease"] as const, "input.change");
    const forwardReactionHeat = oneOf(
      input.forwardReactionHeat,
      ["endothermic", "exothermic"] as const,
      "input.forwardReactionHeat",
    );
  ...
  if (disturbance === "add_inert_gas") {
    const constraint = oneOf(
      input.constraint,
      ["constant_volume", "constant_pressure"] as const,
      "input.constraint",
    );
    const gasReactantMoles = reaction.reactants
      .filter((entry) => entry.phase === "g")
  ...
  let qBasedVerification: Record<string, unknown> | null = null;

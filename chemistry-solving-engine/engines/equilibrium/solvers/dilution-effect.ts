...
export function solveDilutionEffect(input: Record<string, unknown>) {
  ...
  const mode = oneOf(input.mode ?? "add_water", ["add_water", "controlled_concentration"] as const, "input.mode");
  const controlledSpecies = stringArray(input.controlledSpecies, "input.controlledSpecies");
  const dissociatedSide = oneOf(input.dissociatedSide ?? "products", ["reactants", "products"] as const, "input.dissociatedSide");
  ...
  if (mode === "controlled_concentration" && controlledSpecies.length === 0) {
    throw new EngineError(
      "INVALID_INPUT",
      "controlled_concentration 模式必須提供 input.controlledSpecies",
      "input.controlledSpecies",
    );
  }
  ...
  for (const species of controlledSpecies) {
    if (!activeNames.has(species)) {
      throw new EngineError(
        "INVALID_INPUT",
        `受控物種 ${species} 不在反應商的有效物種中`,
        "input.controlledSpecies",
      );
    }
  }

  const postDilutionActivities: Record<string, number> = { ...initialActivities };
  ...
  const effectiveDeltaNu = activeSpecies(reaction).reduce((sum, entry) => {
    if (mode === "controlled_concentration" && controlledSpecies.includes(entry.species)) return sum;

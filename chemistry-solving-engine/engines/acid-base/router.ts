...
import type { EngineRequest, EngineResponse } from "../../core/engine-interface.js";
...
export function solveAcidBase(request: EngineRequest): EngineResponse {
  try {
    const threshold = request.options?.approximationThreshold ?? 0.05;
    let payload;
    switch (request.operation) {
      case "strong_acid_base": payload = solveStrongAcidBase(request.input); break;
      case "weak_acid": payload = solveWeakAcid(request.input, threshold); break;
      case "weak_base": payload = solveWeakBase(request.input, threshold); break;
      case "weak_acid_diprotic": payload = solveDiproticWeakAcid(request.input, threshold); break;
      case "reconstruct_diprotic_equilibrium": payload = reconstructDiproticEquilibrium(request.input); break;
      case "derive_diprotic_constants": payload = deriveDiproticAcidConstants(request.input); break;
      case "neutralization": payload = solveNeutralization(request.input); break;
      case "titration": payload = solveTitration(request.input); break;
      case "buffer": payload = solveBufferOperation(request.input); break;

...
export function solveNeutralization(input: Record<string, unknown>) {
  ...
  if (acid.strength === "weak" && base.strength === "strong") {
    if (!acid.Ka || acid.Ka <= 0) throw new Error("弱酸與強鹼混合時必須提供 acid.Ka");
  ...
  if (acid.strength === "strong" && base.strength === "weak") {
    if (!base.Kb || base.Kb <= 0) throw new Error("強酸與弱鹼混合時必須提供 base.Kb");
...
function mergeChild(method: string, common: Record<string, unknown>, child: any, routedModule: string) {
  return {
    method,
    result: child.result,

...
5. 僅在 User 明確指定「解題規格」或「本機格式路由」時，依指定的三／四步驟或反應表呈現；未指定或資料不足時保留一般詳解，不可為了版型硬湊步驟。計算題含等號的 $…$ 推導行：關鍵數字與因子須用 \htmlData{note=白話短註}{…} 標在式內（規則見 User 的 [NOTE 附錄]）。**計算過程只放數值、符號與運算；每一行等號鏈的最後結果可附單位**，例如 `$C=\dfrac{n}{V}=0.002\,M$`。@@ANSWER@@ 也要附單位。
...
- 計算／問答題：只有指定四步推導時才使用「已知與目標、關係式、代入計算、驗算與結論」標題；「驗算」至少確認單位、量級、正負號或代回關係式其中一項，不能只重複答案。
